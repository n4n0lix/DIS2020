...
public class MovieService extends MovieServiceBase {
	...
	public Document findMovieByTitle(String title) {
		// There is no findOne(..) !?
		Document result = movies.find(new Document("title", title)).first();
		return result;
	}
	...
	public void saveMovieComment(String id, String comment) {
		UpdateResult result = movies.updateOne(
				eq("_id", id),
				set("comment", comment));
	}
	...
	public FindIterable getByTweetsKeywordRegex(String keyword, int limit) {
		// #1 Find movie names
		DistinctIterable<String> tweetsResult = tweets.distinct("movie",
			regex("text", ".*" + keyword + ".*"),
			String.class
		);

		List<String> movieNames = new ArrayList<>();
		for(String r : tweetsResult) {
			movieNames.add(r);
		}
